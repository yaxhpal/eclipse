...
@EXPORT_OK=qw(
	...
	%EXPORT_TAGS = ( all =>[qw( InsertInTable DeleteInTable SearchInTable UpdateInTable GetPrimaryKeys GetRecordCount)]
				);
}
...
=head2 GetRecordCount
	...
	$hashref = &GetRecordCount($tablename,$data,$filtercolumns,$searchtype);
...
sub GetRecordCount{
    my ($tablename, $filters, $searchtype) = @_;
	$searchtype||="exact";
    my $dbh   = C4::Context->dbh; 
    my $sql   = "SELECT COUNT(*) as count from $tablename";
    my $row; 
    my $sth; 
    my ($keys,$values)=_filter_fields($tablename, $filters, $searchtype, undef); 
	if ($keys){
		my @criteria=grep{defined($_) && $_ !~/^\W$/ }@$keys;
		if (@criteria) { 
			$sql.= do { local $"=') AND ('; 
    ...
    eval{$sth->execute(@$values)}; 
	warn $@ if ($@ && $debug);
    my $recordCount = $sth->fetchall_arrayref( {} ); 
    return $recordCount->[0]->{count};
}
