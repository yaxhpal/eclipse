...
my $encryptionKey = C4::Context->get_custom_config('encryptionKey');
...
if (defined($reg_id) && $reg_id ne '') {
	$reg_id = decrypt($reg_id, $encryptionKey);
}
...
$template->param ( test => "Processing..."."\n");
...
if (defined($reg_id)&& $reg_id ne '') {
        ...
        $email, $telephone, $isdt, $stdt, $mobile, $fax, $isdf, $stdf, $agentcode, $reg_id);
		...
		if (defined($couponcode) && $couponcode ne '') {
			...
			($amount, $isValid) = applyCoupon($couponcode, $data);
			$template->param ( test => "Amount: ".$amount." IsValid: ".$isValid."\n");
		}
		$data = $amount if ($isValid); 
		########################################################################################################
...
if ($op eq 'payment'){
	...
	$sth->finish();
	my %args;
	my $opac_base_url = C4::Context->preference('OPACBaseURL'); # Using the base url preference so that we don't have to hardcode the return url.
	
	unless ($branchcode eq 'CB' or $branchcode eq 'KD'){ 
	      %args = (
	                 ...
	                 'vpc_OrderInfo' => "KH$reg_id", #//this also better to be a unique number
	                 'vpc_ReturnURL' => "http://$opac_base_url/cgi-bin/koha/opac-payments.pl",#//Add the return url here so you have to code here to capture whether the payment done successfully or not
	                 'vpc_Version' => "1",
	              );
...
$template->param ( reg_id => encrypt($reg_id, $encryptionKey));
