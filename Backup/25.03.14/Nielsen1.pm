...
sub readMarcRecord {
	...
	my $marcblob = getNielsenMarc21($isbnText) if ($isbnText);
...
#   }
	if ($marcblob) {
		...
		if ($charset_guessed eq 'failed') {
			warn '[Nielsen::readMarcRecord] ERROR: failed to perform character conversion.'."\n";
			return undef;
		}
...
#        #edition statement
#        if ( $field = $marc_record->field('022') ) {
#            my $s = $field->as_string('a');
#            if ($s) {
#                $sfd{x} = $s;
#            }
#        }
#        if ( $field = $marc_record->field('020') ) {
#            my $s = $field->as_string('a');
#            if ($s) {
#                $sfd{z} = $s;
#            }
#        }
#        if ( $field = $marc_record->field('001') ) {
#            $sfd{w} = $field->data(),;
print $marc_record->as_xml();

print Dumper GetMarcAuthors $marc_record, 'MARC21-Nielsen';
print Dumper GetMarcAuthors $marc_record, C4::Context->preference("marcflavour");
	}
...
sub updateMarcRecord {
	my $isbnText = shift;
	my $isbn 	 = Business::ISBN->new($isbnText);
	my $marc = getNielsenMarc21($isbnText) if ($isbnText);
	if ($marc) {
		my ($marc_record, $charset_guessed, $char_errors) = MarcToUTF8Record($marc, C4::Context->preference("marcflavour"));
		if ($charset_guessed eq 'failed') {
			warn '[Nielsen::getNielsenMarc21] ERROR: failed to perform character conversion.'."\n";
			return undef;
		}
		my %data = ('marc'=>$marc, 'marcxml'=>$marc_record->as_xml());
