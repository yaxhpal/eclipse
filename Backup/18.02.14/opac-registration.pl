...
my $reg_id = undef;
if ($op) { 
# Get the registration id from the session if any
 $reg_id =  getRegSession()->param('reg_id');
 $template->param('test1'	=> $op);
} else {
 getRegSession()->param('reg_id' => undef);
 $template->param('test1'	=> "Op is undef");
}
...
# Store registration id into global session
getRegSession()->param('reg_id' => $reg_id);
...
$template->param('test'	=> Dumper(getRegSession()));
